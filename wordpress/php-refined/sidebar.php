...
if ( $post->post_parent) {
	...
	if ( $grandparent ) {
		...
		if ( $greatgrand ) {
			...
			if ( $children ) {
				...
				echo ('<ul>'.$children.'</ul></div>'); //output children
			}		
		} else {
			...
			if ( $children ) {
				echo ('<h3 class="sidebar-nv"><a href="/'.$parent_slug.'">'.$title.'</a></h3>');//output parent link at H5
		...
		$title = get_the_title( $post->post_parent ); //get the title for the parent post
		if ( $children ) {
			echo ('<h5 class="sidebar-nv"><a href="/'.$parent_slug.'">'.$title.'</a></h5><hr>');//output parent link at H4
			echo ('<div id="list"><ul>'.$children.'</ul></div>'); //output children
		}		
	}
} else {
	...
	if ( $children ) {
		echo ('<h2 class="sidebar-nv"><a href="/'.$slug.'">'.$title.'</a></h2>');//output the parent link at H4
